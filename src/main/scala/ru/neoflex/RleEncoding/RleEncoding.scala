...
class RleEncoding {
  ...
  def encode(str: String): Seq[Block] = {
    ...
    var collectionBlockHelper = new ListBuffer[Block]
    val (_, optBlock, result) = {
      str.toCharArray.foldLeft((None: Option[Char], None: Option[Block], Seq.empty[Block])) {
        ...
        case ((None, None, result), char) if str.length == 1 =>
          collectionBlockHelper += UncompressedBlock(1, Seq(char)); (Some(char), Some(UncompressedBlock(1, Seq(char))), result)
        ...
        case ((Some(prev), Some(block@UncompressedBlock(_, _)), result), char) =>
          collectionBlockHelper += UncompressedBlock(block.length - 1, block.data.dropRight(1)); (Some(char), Some(CompressedBlock(2, prev)), result :+ block)
        ...
        case ((Some(prev), Some(block@CompressedBlock(_, _)), result), char) if prev != char =>
          collectionBlockHelper += block; (Some(char), None, result :+ block)
      }
    }

    if (str.length > 1) {
      if (str(str.length - 2) == str(str.length - 3) && str(str.length - 1) != str(str.length - 2)) {
        collectionBlockHelper += UncompressedBlock(1, Seq(str(str.length - 1)))
      }
      else {
        collectionBlockHelper += optBlock.get
      }
    }
    collectionBlockHelper.toSeq
  }
}
